...
// writeFile creates path, and the directories above it, with the given content.
...
func TestRestoreKeepsFoldersOnDisk(t *testing.T) {
	...
	writeFile(t, filepath.Join(folder, "a.md"), "# x\n")
	writeFile(t, single, "# x\n")

	want := &State{
		Files:   []StateFile{{Path: single}, {Path: filepath.Join(folder, "a.md")}},
		Folders: []WatchedFolder{{Path: folder, Recursive: false}},
	}
	if err := saveState(want); err != nil {
		t.Fatal(err)
	}

	newHubWithin(t, 5*time.Second) // restores, then saves once
...
func TestRemoveEmptyFollowedFolderPersists(t *testing.T) {
	...
	if err := h.FollowFolder(&WatchedFolder{Path: folder}); err != nil {
		t.Fatal(err)
	}
